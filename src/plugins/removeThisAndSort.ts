...
export const removeThisAndSort: ASTTransform = (astResults, options) => {
  ...
  const transformer = (): ts.TransformerFactory<ts.Node> => {
    return (context) => {
          ...
          if (node.expression.kind === tsModule.SyntaxKind.ThisKeyword) {
            ...
            if (refVariables.includes(propertyName)) {
              ...
              return tsModule.factory.createPropertyAccessExpression(
                tsModule.factory.createIdentifier(propertyName),
                tsModule.factory.createIdentifier("value"),
              );
            } else if (domeRefVariables.includes(propertyName)) {
              dependents.push(propertyName);
              return tsModule.factory.createNonNullExpression(
              ...
              return addTodoComment(
                ...
                tsModule.factory.createPropertyAccessExpression(
                  tsModule.factory.createPropertyAccessExpression(
                    tsModule.factory.createIdentifier(options.setupContextKey),
                    tsModule.factory.createIdentifier("root"),
                  ),
                  tsModule.factory.createIdentifier(propertyName),
                ),
      ...
      return (node) => tsModule.visitNode(node, removeThisVisitor) as ts.Node;
    };
  };
  ...
  const astResultNoDependents = transformResults.filter(
    (el) => el.nodeDependents.length === 0,
  );
  let otherASTResults = transformResults.filter(
    (el) => el.nodeDependents.length !== 0,
  );
  ...
  do {
    ...
    otherASTResults = otherASTResults.filter((el) => {
      if (
        el.nodeDependents.every((dependent) =>
          resultHaveDependents.includes(dependent),
        )
      ) {
        result.push(el);
        hasPush = true;
